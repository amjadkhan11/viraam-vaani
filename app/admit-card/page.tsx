...
export default function StudentAdmitCardPortal() {
  ...
  const handleFetchAdmitCard = async (e: React.FormEvent) => {
    ...
    if (loggedInEmail && email.trim().toLowerCase() !== loggedInEmail.trim().toLowerCase()) {
      Swal.fire({
        ...
        customClass: { popup: "rounded-3xl bg-white text-slate-900 border border-blue-500" }
      });
  ...
  if (pageLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 via-white to-blue-50 text-slate-900">
        <RefreshCw className="animate-spin text-blue-700 mb-2" size={32} />
        <p className="font-bold text-slate-700 uppercase tracking-wider text-xs">Aapki details verify ho rahi hain...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-white to-blue-50 p-4 md:p-6 text-slate-900 flex flex-col items-center print-wrapper relative overflow-hidden font-sans">
      
      {/* Light Mesh Accent Blurs (Hero Blue Theme) */}
      <div className="absolute top-20 left-20 h-80 w-80 rounded-full bg-blue-300/20 blur-3xl pointer-events-none print:hidden" />
      <div className="absolute bottom-20 right-20 h-80 w-80 rounded-full bg-blue-400/20 blur-3xl pointer-events-none print:hidden" />

      {/* SEARCH CARD */}
      <div className="w-full max-w-xl bg-white rounded-3xl border border-slate-200/80 p-6 shadow-xl mb-6 print:hidden relative z-10">
        <div className="text-center mb-6">
          <div className="w-12 h-12 bg-blue-50 border border-blue-200 text-blue-700 rounded-2xl flex items-center justify-center mx-auto mb-3 shadow-sm">
            <FileText size={24} />
          </div>
          <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-wide">
            DOWNLOAD <span className="text-blue-700">ADMIT CARD</span>
          </h1>
          <p className="text-xs font-bold text-blue-700 bg-blue-100/80 border border-blue-200 px-3 py-1.5 rounded-lg inline-block mt-2 shadow-sm">
        ...
        <form onSubmit={handleFetchAdmitCard} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 focus:border-blue-700 outline-none rounded-xl py-3.5 pl-12 pr-4 text-sm text-slate-900 font-medium transition-all shadow-sm"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-700 to-blue-500 font-bold text-sm tracking-wide text-white flex items-center justify-center gap-2 hover:scale-[1.02] transition-all active:scale-[0.99] disabled:opacity-50 shadow-md cursor-pointer border-none outline-none"
          >
      ...
      {admitCardData && (
        <div className="w-full max-w-4xl flex flex-col items-center animate-fadeIn printable-area relative z-10">
          
          {/* Quick Print Action Bar */}
          <div className="w-full bg-white border border-slate-200/80 rounded-2xl p-3 mb-4 flex items-center justify-between shadow-md print:hidden">
            <span className="text-xs text-blue-700 font-semibold flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-blue-600 animate-pulse" /> Verify all information before printing.
            </span>
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-700 to-blue-500 text-xs font-bold text-white tracking-wide transition-all shadow-md hover:scale-105 active:scale-95 cursor-pointer border-none outline-none"
            >
          ...
          <div className="admit-card-container w-full bg-white text-slate-900 rounded-2xl p-6 md:p-8 border-4 border-double border-blue-700 shadow-xl font-sans relative overflow-hidden print:border-2 print:border-blue-700 print:p-5 print:shadow-none print:rounded-none">
            
            {/* Background Watermark */}
            <div className="absolute inset-0 flex items-center justify-center opacity-[0.05] pointer-events-none select-none z-0 overflow-hidden watermark-container">
              <h1 className="text-6xl md:text-8xl font-black tracking-widest text-slate-900 -rotate-12 uppercase text-center whitespace-nowrap">
                VIRAAM VAANI
              </h1>
            </div>
            ...
            <div className="relative z-10">
              {/* Header */}
              <div className="flex flex-row items-center justify-between border-b-[3px] border-blue-700 pb-3 mb-4 header-section">
                <div className="flex items-center flex-1 relative">
                  {/* Logo */}
                  <div className="w-14 h-14 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-center overflow-hidden shrink-0 shadow-sm print:w-12 print:h-12">
                    <img
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 text-center">
                    ...
                    <p className="text-[10px] md:text-xs text-blue-700 font-bold uppercase tracking-wider">
                ...
                <div className="text-right">
                  <span className="inline-block px-2.5 py-1 bg-blue-100 text-blue-700 text-[9px] font-black rounded-md uppercase tracking-widest border border-blue-200">
                    ORIGINAL PASS
                  </span>
                  <p className="text-[11px] font-bold text-slate-700 mt-1">Ref: <span className="font-mono text-blue-700">{admitCardData.registrationNo || 'N/A'}</span></p>
                </div>
              </div>
              ...
              <div className="flex flex-row gap-4 items-start mb-4 photo-details-row">
                <div className="card-sections-wrapper flex flex-col md:flex-row gap-4 flex-grow text-xs">
                  ...
                  {/* Student Details */}
                  <div className="student-box flex-1 border border-slate-200 rounded-xl p-3.5 bg-slate-50/50 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-blue-700"></div>
                    <h3 className="text-[11px] font-black text-blue-700 uppercase tracking-widest border-b border-slate-200 pb-1 mb-2">
                      Student Particulars
                    </h3>
                    <div className="space-y-1">
                      <p className="flex justify-between border-b border-slate-100 pb-0.5"><strong className="text-slate-500 font-medium">Full Name:</strong> <span className="font-black text-slate-900 text-xs uppercase">{admitCardData.studentName}</span></p>
                      ...
                      <p className="flex justify-between"><strong className="text-slate-500 font-medium">Class:</strong> <span className="font-bold text-slate-900">{admitCardData.batch}</span></p>
                    </div>
                  </div>

                  {/* Exam Details */}
                  <div className="logistics-box flex-1 border border-slate-200 rounded-xl p-3.5 bg-slate-50/50 relative overflow-hidden">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="flex justify-between border-b border-slate-100 pb-0.5"><strong className="text-slate-500 font-medium">Date:</strong> <span className="font-bold text-blue-700">{admitCardData.examDate} ({admitCardData.examDay})</span></p>
                ...
                {/* Photo Box */}
                <div className="w-28 h-36 border-2 border-dashed border-slate-300 bg-slate-50 rounded-xl flex flex-col items-center justify-center p-2 text-center shrink-0 photo-box-container select-none">
              ...
              {/* Venue Info */}
              <div className="w-full border border-slate-200 bg-slate-50/50 rounded-xl p-3 mb-4 text-xs">
                <div className="leading-normal">
                  <strong className="text-slate-500 font-bold uppercase tracking-wider block mb-0.5">Test Venue / परीक्षा केंद्र:</strong>
                  <span className="font-black text-slate-900 text-sm block">{admitCardData.centerName}</span>
                ...
                {(admitCardData.roomNo || admitCardData.seatNo) && (
                  <div className="flex gap-6 mt-2 pt-1.5 border-t border-slate-200 font-bold text-slate-900">
                    <p>Room No: <span className="text-blue-700">{admitCardData.roomNo || "—"}</span></p>
                    <p>Seat No: <span className="text-blue-700">{admitCardData.seatNo || "—"}</span></p>
                  </div>
                )}
              </div>

              {/* Instructions */}
              <div className="border border-blue-200 rounded-xl p-3 bg-blue-50/40 mb-4 page-break-avoid">
                <div className="flex items-center gap-2 mb-1 border-b border-blue-200 pb-1">
                  <ShieldAlert className="text-blue-700 shrink-0" size={14} />
                  <h4 className="text-[11px] font-black uppercase text-blue-900 tracking-wider">महत्वपूर्ण निर्देश / Candidate Instructions</h4>
                </div>
                <ul className="list-decimal list-inside text-[10px] text-slate-700 font-semibold space-y-0.5 leading-relaxed pl-1">
                  <li> परीक्षा केंद्र पर इस <strong className="text-blue-700">Admit Card</strong> को प्रिंट कराकर साथ लाना अनिवार्य है।</li>
              ...
              <div className="flex justify-between items-end pt-2 text-center text-xs text-slate-600 page-break-avoid summary-footer">
                ...
                <div>
                  ...
                  <div className="w-44 border-b-2 border-slate-900 mx-auto mb-1 h-16 flex items-end justify-center relative pb-0.5">
                    ...
                    <div className="absolute w-36 h-36 -rotate-12 pointer-events-none select-none z-20 -top-10 -left-1 opacity-80 mix-blend-multiply">
                      <svg viewBox="0 0 200 200" className="w-full h-full text-[#c53030] fill-current">
                        <defs>
                  ...
                  <p className="font-black text-slate-900 text-[10px]">Authorized Signatory</p>
                </div>
              </div>
            </div>
